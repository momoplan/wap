...
%><a href="<%=response.encodeURL(path+"/wap/array5/singleAutoSelection.jspx")%>">排列五单式机选</a><%
 	out.print(CommonUtil.printChar());
 %>注码修改<br />
 	      	  您的投注：<br/>
			排列五第<%
	...
	%>
	<form action="/array5/modifyBetDetail.jspx" method="post" style="margin:0px;display:inline;">
	...
	%> <%
		...
		for(int i=0;i<s.length;i++){
			xgzhuma = s[i];
	%>
		<input name="i<%=i%>" type="text" emptyok="true"  size="9" maxlength="9" format="*N" tabindex="1" value="<%=xgzhuma%>"/><br/>
	<%	
		}
	%>
