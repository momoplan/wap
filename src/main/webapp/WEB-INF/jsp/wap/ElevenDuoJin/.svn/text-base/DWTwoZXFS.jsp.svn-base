...
<a href="/wap/wapindex.jspx">首页</a>-<a href="<%=response.encodeURL(path+"/wap/buyLottery.jspx")%>"><%
		...
		选前二：<a href="/wap/ElevenDuoJin/ForwardTwoZX.jspx">直选单式</a>|<a 
			href="/wap/ElevenDuoJin/ForwardTwoZXFS.jspx">直选复式</a>|直选定位复式<br/>
		...
		%> <%
 		...
 		%> <%
 			out.print(deadline);
 		%> <%}%>
 		<form action="/wap/ElevenDuoJin/DWTwoZXFS.jspx" method="post">
		...
		<%} %>
 		...
 		<form action="/elevenduojin/groupDetail.jspx" method="post">
	 		...
	 		<input type="hidden" name="method" value="groupDetail" />
	 		<input type="hidden" name="type" value="z2dwfs" />
