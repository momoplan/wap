...
<a href="/wap/wapindex.jspx">首页</a>-<a href="<%=response.encodeURL(path+"/wap/buyLottery.jspx")%>"><%
		...
		%> <%
 		...
 		%> <%
 			out.print(deadline);
 		%> <%}%>
 		<form action="/wap/ElevenDuoJin/ForwardTwoZX.jspx" method="post">
 		  <input type="submit" value="刷新" />
 		</form><br />
 		从1~11选择,注码个数为两个(第一位+第二位=2),10之前号码前面补0,如1要输入01<br/>
 		<%if(message != null){ %>
		<span style="color: red;">   提示： <%out.println(message); %><br/></span>
		<%} %>
 		选前二直选单式<br/>
 		<form action="/elevenduojin/groupDetail.jspx" method="post">
	 		第一位：<input name="first" type="text" emptyok="true"  size="20" maxlength="20" format="*N" tabindex="1" value="<%=first %>" /><br/>
	 		第二位：<input name="second" type="text" emptyok="true"  size="20" maxlength="20" format="*N" tabindex="1" value="<%=second %>" /><br/>
	 		...
	 		<input type="hidden" name="type" value="z2" />
