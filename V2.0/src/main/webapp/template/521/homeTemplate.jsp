...
<body>
<div class="logo_box"> 
	<img src="http://219.148.162.70:8000/w/wap/images/hlxkLOGO.png" alt="" class="logo"></img> </div>
<div style="margin:5px; border:1px solid #cc9999; background:#f8f8f8; padding:10px; font-size:12px">您好，您现在访问的是互联星空与如意彩合作的彩票频道。在此购买彩票，您无需注册即可登陆购彩，系统默认您以您的手机号码为购彩账号（您的相关信息不会泄露）。祝您使用愉快。</div>
...
<!--	<%if(topForHLXKBean.getTitle()!=null&&!"".equals(topForHLXKBean.getTitle())){
	...
	<%}%>-->
	<decorator:body />
<%if(infoList.get(0).getTitle()!=null&&!"".equals(infoList.get(0).getTitle())){%><h1>【星空热点】</h1><div class="contant"> 
<%
		int info = infoList.size();
		  int arr[] = new int[info];
		  for(int i = 0;i < arr.length;i++) {//外层的for循环 用来控制对数组赋值的索引的变化
		   while(true){
...
<h1>【热点频道】</h1><div class="contant">
	...
	%>
	<a href="http://wap.vnet.mobi">返回互联星空</a></div>
</body>
</html>
