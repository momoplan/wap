...
<%
    ...
    String autoFScode = (String) request.getAttribute("autoFScode");if(autoFScode == null) autoFScode = "1";

%>
	<title>十一运夺金任选一</title>
<body>
<a href="/wap/wapindex.jspx">首页</a>-<a href="<%=response.encodeURL(path+"/wap/buyLottery.jspx")%>"><%
		...
		任一|<a 
		  ...
		  <a 
		   href= "/w/wap/ElevenDuoJin/optionOne.jspx">自选</a>|<a 
		   href="/w/wap/ElevenDuoJin/optionOneAutoDS.jspx">单式机选</a>|复式机选<br/>
		<%
		...
		%> <%
 		...
 		%> <%
 			out.print(deadline);
 		%> <%}%>
 		<form action="/wap/ElevenDuoJin/optionOneAutoFS.jspx" method="post">
...
<span style="color: red;">   提示： <%out.println(message); %><br/></span>
 <%} %>
		...
		机选:<input name="autoFScode" type="text"  size="10" maxlength="2"  tabindex="1" value="<%=autoFScode %>" />个(1-11)<br/>
		...
		<input type="hidden" name="type" value="autoR1" />
