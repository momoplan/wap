...
%>  
<title>用户反馈回复修改</title>
<body>

<a href="/wap/wapindex.jspx">首页</a>-用户反馈回复修改<br/>
<%
	if (message != null) {
%> <a style="color: red"><%
...
<br/><br/>
<%if(feedBack.getAnswer()!=null&&mess==null){%>
	<form action="<%=response.encodeURL(path + "/userWap/updateAnswerFeedBack.jspx" )%>" method="post">
	修改回复:<br/>
	<textarea name="answer" rows="3" cols="20"><%=feedBack.getAnswer() %></textarea><br/>
