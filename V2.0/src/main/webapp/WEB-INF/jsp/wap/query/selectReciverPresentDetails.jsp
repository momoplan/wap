...
-<a href="/w/wap/query/selectGift.jspx">赠送查询</a>-被赠送彩票<br/><br/>
<c:if test="${not empty presents }">
		<c:forEach items="${presents }" var="present">
			好友用户名:${present.userName }<br />
			购买金额:${present.amt }<br />             
			...
			<c:if test="${upPage }">
		<c:url value="/BetingSelectAction/selectReciverPresentDetails.jspa" var="start">
			<c:param name="startLine" value="0"></c:param>
			<c:param name="endLine" value="${endLine }"></c:param>
		</c:url>
		<a href="${start }">首页</a>
		<c:url value="/BetingSelectAction/selectReciverPresentDetails.jspa" var="up">
	...
	<c:if test="${nextPage }">
		<c:url value="/BetingSelectAction/selectReciverPresentDetails.jspa" var="next">
			<c:param name="startLine" value="${endLine*nowPage }"></c:param>
			<c:param name="endLine" value="${endLine }"></c:param>
		</c:url>
		<a href="${next }">下一页</a>
		<c:url value="/BetingSelectAction/selectReciverPresentDetails.jspa" var="end">
