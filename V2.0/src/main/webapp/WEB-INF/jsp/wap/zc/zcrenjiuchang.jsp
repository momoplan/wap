...
<c:if test="${not empty batchs.bactch0}">
 <a href="<%=request.getContextPath()%>/zc/zcduizhen.jspx?lotno=T01004">当前${batchs.bactch0}期</a>
 </c:if>
 <c:if test="${not empty batchs.bactch1}">
 |<a href="<%=request.getContextPath()%>/zc/zcduizhen.jspx?lotno=T01004&batchcode=${batchs.bactch1}&overtime=${batchs.bactch1endtime}">当前${batchs.bactch1}期</a>
 </c:if>
 <c:if test="${not empty batchs.bactch2}">
 |<a href="<%=request.getContextPath()%>/zc/zcduizhen.jspx?lotno=T01004&batchcode=${batchs.bactch2}&overtime=${batchs.bactch1endtime}">当前${batchs.bactch2}期</a>
 </c:if>
 <c:if test="${not empty batchs.bactch3}">
 |<a href="<%=request.getContextPath()%>/zc/zcduizhen.jspx?lotno=T01004&batchcode=${batchs.bactch3}&overtime=${batchs.bactch1endtime}">当前${batchs.bactch3}期</a>
 </c:if>
 <br>
...
<div>
   *3为胜，1为平，0为负。多选无间隔，如胜平，则填31
	...
	<div class="zucai_tr1">
		...
		<input type="text" name="betcode${infos.tempId}" maxlength="3" size="35">
		<br>
		<span>均值：${infos.avgOdds}</span>
	</div>
<input type="hidden" name="duizhens${infos.tempId}" value="${infos.tempId}|${infos.HTeam }vs${infos.VTeam}"/>
	<input type="hidden" name="lotno" value="T01004">
	<div class="jc_15"></div>
	 ...
	 <c:if test="${batchcode == null}">
		...
		【设置倍数】<br>
	<input name="beishu" value="" type="text">倍(最大10000倍)<br>
