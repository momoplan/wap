...
<body>
	...
	<%
		...
		单式机选|<a href="/wap/qilecai/MultipleAutoSelection.jspx">复式机选</a>|<a href="/wap/qilecai/DingDanAutoSelection.jspx">定胆机选</a><br/>
			<%if (deadline!=null) { %>
	...
	<form
		action="<%=response.encodeURL(path+"/qilecai/singleAutoSelection.jspx")%>"
		method="post">
		机选注数:<input name="zhushuStr" type="text" emptyok="true" maxlength="3"
			size="3" format="*N" value="<%=zhushuStr %>" /><br /> 倍数:<input
			name="beishu" type="text" emptyok="true" maxlength="2" size="2"
			format="*N" value="<%=beishu %>" />(最多50倍)<br />
		<%if (rbint.getString("addNumberSwitch").equals("1")) { %>
		追号:<input name="addNumber" type="text" emptyok="true" maxlength="2"
			size="2" format="*N" value="<%=addNumber %>" />期<br />
		(最高99期,默认追1期,即买当前期)<br />
		<%} %>
		<input type="hidden" name="type" value="JXDS" /> <input type="submit"
			value="提交投注" />
	</form>
	...
	<% } %>
	<a
		href="<%=response.encodeURL(path + "/wap/qilecai/QilecaiIndex.jspx"
								)%>">返回上一页</a>
</body>
