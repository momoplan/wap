...
<a href="/wap/wapindex.jspx">首页</a>-<a href="<%=response.encodeURL(path+"/wap/buyLottery.jspx")%>"><%
		...
		自选|<a 
		   href="/w/wap/11select5/optionFourAutoDS.jspx">单式机选</a>|<a 
		   href="/w/wap/11select5/optionFourAutoFS.jspx">复式机选</a><br/>
		<%
		...
		%> <%
 		...
 		%> <%
 			out.print(deadline);
 		%> <%}%>
 		<form action="/wap/11select5/optionFour.jspx" method="post">
		...
		%></a>
 		01-11选4-7个数字,号码间无分隔,数字不可重复,小于10则补0,如选择2则填写02<br/>
 		自选<br/>
 		<form action="/11select5/optionByHand.jspx" method="post">
 		注码：<input name="zhuma" type="text" emptyok="true"  size="20" maxlength="14" format="*N" tabindex="1" value="<%=zhuma %>" /><br/>
